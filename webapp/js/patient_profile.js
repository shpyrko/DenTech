        ...
        for (var item in patient_info) {
            if (typeof patient_info[item] === "object") {
                for (var condition in patient_info[item]) {
                    document.getElementById("conditions").insertAdjacentHTML('beforeend', '<h1 class="text-primary">' + condition +  ': </h1> <h1> ' + convert_to_yes_no(patient_info[item][condition]) + '</h1><br />');
                }
            }
            else {
                document.getElementById("basic_info").insertAdjacentHTML('beforeend', '<h1 class="text-primary">' + item + ': </h1> <h1>' + patient_info[item] + '</h1><br />');
            }
        }
